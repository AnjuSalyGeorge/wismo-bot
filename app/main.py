...
MAX_MESSAGE_CHARS = 2000  # basic abuse guard


def _extract_llm_fields(actions: List[Dict[str, Any]]) -> Dict[str, Any]:
    intent = None
...
@app.post("/chat")
def chat(req: ChatRequest, request: Request, caller=Depends(require_api_key)):
    """
    Protected endpoint (API key required unless API_KEY env not set).
    """
...
def ui_chat(req: ChatRequest, request: Request):
    """
    Public UI endpoint (no key required in browser).
    """
